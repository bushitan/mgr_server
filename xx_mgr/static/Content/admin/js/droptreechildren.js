...
layui.define(['jquery', 'layer'], function (exports) {
    ...
    var hideMenu = function () {
        $("#childmenuContent").fadeOut("fast");
        $("body").unbind("mousedown", onBodyDown);
    }

    //滚动条下拉
    function onBodyDown(event) {
        if (!(event.target.id == "childmenuContent" || $(event.target).parents("#childmenuContent").length > 0)) {
            hideMenu();
        }
    }
    ...
    Class.prototype.render = function () {
        ...
        $.getJSON(this.config.url,
            ...
            function (json) {
                ...
                zTreeObj = $.fn.zTree.init($("#childorg"), setting, json);
                that.setCheck();
    ...
    exports('droptreechildren', function (url, name, id, selectedMulti) {
      var options = {
